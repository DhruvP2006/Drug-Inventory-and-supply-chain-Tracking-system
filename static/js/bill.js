...
function calculateTotal(row) {
  ...
  const rate =
    parseFloat(row.querySelector('input[placeholder="Rate"]').value) || 0;
  const discount =
    parseFloat(row.querySelector('input[placeholder="Discount %"]').value) || 0;
...
function downloadBill() {
  ...
  doc.text(
    "For any queries, contact us at info@yourcompany.com",
    14,
